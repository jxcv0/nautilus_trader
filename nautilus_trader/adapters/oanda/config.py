...
class OandaDataClientConfig(LiveDataClientConfig, frozen=True):
    ...
    bearer_token : str, [default=None]
        The OANDA api bearer token.
        If ``None`` then will source the `OANDA_API_KEY` environment variable.
    base_url_http : str, optional
        The base url to OANDA's HTTP API.
        If ``None`` then will use the default live URL.
    demo : bool, default False
        If the client is connecting to the practice testnet.
